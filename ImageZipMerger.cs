...
class Merger : Form {
 EpisodeListBox episodes = new EpisodeListBox(); ListBox pages = new ListBox();
 ...
 public Merger() {
  Text="이미지 ZIP 합치기 v1.8"; Size=new Size(1180,800); MinimumSize=new Size(950,620); Font=new Font("맑은 고딕",10); StartPosition=FormStartPosition.CenterScreen; BackColor=Color.FromArgb(245,247,250); AutoScaleMode=AutoScaleMode.Font;
  ...
  excluded.Text="제외 이미지 표시";excluded.AutoSize=false;excluded.Dock=DockStyle.Fill;excluded.Margin=new Padding(0);pgControls.Controls.Add(excluded,0,1);excluded.CheckedChanged+=delegate {RefreshPages();};pgControls.Controls.Add(Hint("선택 후 Delete로 삭제\n상단 ‘삭제 취소’로 되돌리기"),0,2);
  ...
  EnableFileTips(episodes); EnableFileTips(pages);
  ...
  pages.SelectedIndexChanged+=delegate {if(!refreshing)ShowPreview();}; pages.KeyDown+=delegate(object s,KeyEventArgs e) {if(e.KeyCode==Keys.Delete) Delete(); if(e.Control && e.KeyCode==Keys.A) {for(int i=0;i<pages.Items.Count;i++) pages.SetSelected(i,true); e.SuppressKeyPress=true;}};
 ...
 void RefreshEpisodesAtPosition(int selected,bool keepPosition) {
  int top=episodes.TopIndex;var previous=Current;
  ...
  } finally {episodes.EndUpdate();refreshing=false;}
 ...
 void RefreshPagesAtPosition(bool keepPosition) {
  int top=pages.TopIndex,index=pages.SelectedIndex;var previous=pages.SelectedItem;
  fileTips.SetToolTip(pages,"");refreshing=true;pages.BeginUpdate();
  try {
   pages.Items.Clear();if(Current!=null)pages.Items.AddRange(Current.Pages.Where(p=>excluded.Checked || !p.Removed).ToArray());
  ...
  } finally {pages.EndUpdate();refreshing=false;}
